...
namespace DDD.Application.Service
{
    public interface IBaseServices<T> where T : class,new()
    {
        ...
        /// <returns></returns>
        PageData<T> FindAll<S>(int PageIndex, int PageSize, ISpecification<T> specification, Expression<Func<T, S>> orderByExpression, bool IsDESC, bool? AsNoTracking = true);
