 ...
 * Version: 1.3 (1st April, 2007) - Added setting up Transparent/opaque mode, fix for width and height in %
...
infosoftglobal.FusionCharts.prototype = {
	...
	setDataXML: function(strDataXML){
       ...
       setDataJSON:function(json){
            ...
            this.parseJSON(this.XML.documentElement,json);
            var strXML;
            if(typeof XMLSerializer=="function"){
               strXML=(new XMLSerializer()).serializeToString(this.XML);
            }
            else{
               strXML=this.XML.xml;
            }
            strXML=strXML.replace(/\'/g,"%26apos;");
            strXML=strXML.replace(/\"/g,"'");
            
            this.setDataXML(strXML);
	},
        getXML:function(){
           if(typeof XMLSerializer=="function"){
               strXML=(new XMLSerializer()).serializeToString(this.XML);
	...
	parseJSON:function(DOMElement,part_json){
            var attrib,newel;

            for(var i in part_json){
                
                if(typeof part_json[i]=="object"){
                    if(part_json[i].hasOwnProperty("length")){
                        for(var j in part_json[i]){
                            newel=this.XML.createElement(i);
                            this.parseJSON(newel,part_json[i][j]);
                            DOMElement.appendChild(newel);
                        }
                    }else{
                          newel=this.XML.createElement(i);
                          this.parseJSON(newel,part_json[i]);
                          DOMElement.appendChild(newel);
                    }
                    
                }else{
                    if(typeof part_json[i]=="string" || typeof part_json[i]=="number" || typeof part_json[i]=="boolean"){	
                        attrib=this.XML.createAttribute(i);
                        if(typeof part_json[i]=="boolean"){
                            part_json[i]=(part_json[i]?"1":"0")
                        }
                        attrib.nodeValue=part_json[i];
        ...
        newXMLDocument:function(rootTagName) {
          if (!rootTagName) rootTagName = "chart";
